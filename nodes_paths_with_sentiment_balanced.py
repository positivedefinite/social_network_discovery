...
for i in range(0,len(trees)):
    ...
    if len(tree)>10: # take only trees bigger than 10
        import random
        if random.random()>0.95: #once every 20 times print progress
...
for i in range(0,len(paths)):
    if float(ps[i])<0.5:# all neg are taken
        temp_ps.append(ps[i])
...
with open('paths_with_sentiment_balanced.pkl', 'wb') as f:
    pickle.dump([temp_paths,temp_ps], f)

posCounter = 0
for i in range(0,len(temp_paths)):
    if float(temp_ps[i])>0.7:
        posCounter +=1
